...
<table border="1">
<thead> <tr><th>Name</th><th>Return on Investment</th><th>Start Date</th><th>End Date</th><th>Type</th><th>Custom Attributes</th> </tr></thead>
<tfoot>
%for x in items:
<tr><td>{{x.desc}}</td><td>{{x.roi}}</td>
<td>{{datetime.datetime.fromtimestamp(x.startTs)}}</td><td>{{datetime.datetime.fromtimestamp(x.endTs)}}</td>
<td>{{x.campaign_desc}}</td><td>{{x.attrs}}</td>
<td><a href={{"/campaigns/"+str(x.id)}}>Edit</a></td>
<td><a class="delete" href={{"/campaigns/"+str(x.id)+"/destroy"}}>Delete</a></td></tr>
%end
</tfoot>
</table>
<a href="/campaigns/new">Create New Campaign</a>  <a href="/campaigns/logout">Logout</a>
