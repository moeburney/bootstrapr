...
class chat(Base):
    ...
    profile = relationship("profile", back_populates="chats",lazy='subquery')
    replies = relationship("chat", backref=backref("topic", remote_side=[id], order_by="chat.ts"),lazy='subquery')
...
class feedback(Base):
    ...
    details = Column(Text,default="{}")
    profile = relationship("profile", back_populates="feedbacks",lazy='subquery')
    def update(self, params, session):
        self.id = params.get('id') if 'id' in params else self.id
        self.uuid = params.get('uuid') if 'uuid' in params else self.uuid
        self.profile_id = params.get('profile_id') if 'profile_ud' in params else self.profile_id
...
class profile(Base):
    ...
    campaign = relationship("campaign", backref="profile", secondary=link_table,order_by="campaign.startTs",lazy='subquery')
